  ...
  <v-container fluid class="pa-0">
    <v-row align="center" justify="center" class="hero-section">
      <v-col cols="12" md="8" class="text-center">
        <h1 class="text-h2 font-weight-bold mb-4">Unlock the Power of Knowledge</h1>
        <p class="text-h5 font-weight-regular mb-8">
          Your journey to discovery starts here. Explore endless insights, resources, and knowledge to fuel your growth.
        </p>
        <v-text-field
          v-model="searchQuery"
    ...
    <v-container>
      ...
      <v-window v-model="activeTab">
        <v-window-item value="recent">
          ...
          <v-row>
            <v-col v-for="resource in resources" :key="resource.id" cols="12" sm="6" md="4">
                ...
                <v-card-text>
                  <v-row align="center" class="mx-0">
                    <v-rating
                      ...
                      readonly
                    ></v-rating>
                    <div class="text-grey ms-2">{{ resource.reviews }} Reviews</div>
                  </v-row>
                </v-card-text>
                <v-card-actions>
                  <v-btn color="grey-darken-3" variant="outlined" @click="viewResource(resource.id)">
                    View More
                  </v-btn>
                </v-card-actions>
              </v-card>
...
const tabTitle = computed(() => {
  return activeTab.value === 'recent' ? 'Latest Knowledge Updates' : 'Most Popular Resources'
})
